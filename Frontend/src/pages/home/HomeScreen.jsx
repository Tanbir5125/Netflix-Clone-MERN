...
import useGetTrendingContents from "../../hooks/useGetTrendingContents"
...
const HomeScreen = () => {
  const { trendingContent } = useGetTrendingContents()
  ...
  return (
    <>
      <div className="relative h-screen text-white">
        ...
        {imgLoading && (
          <div className="absolute top-0 left-0 w-full h-full bg-black/70 flex justify-center items-center shimmer -z-10" />
        )}
        ...
        <div className="absolute top-0 left-0 w-full h-full flex flex-col justify-center px-8 md:px-16 lg:px-32">
          ...
          <div className="max-w-2xl">
            <h1 className="mt-t text-6xl font-extrabold text-balance">{trendingContent?.title || trendingContent?.name}</h1>
            <p>{trendingContent?.release_date?.split("-")[0] || trendingContent?.first_air_date?.split("-")[0]}{' '}|{' '}{trendingContent?.adult ? "R-rated" : "PG-13"}</p>
